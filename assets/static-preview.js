(function () {
  ...
  document.addEventListener('click', function (event) {
    ...
    if (trigger.matches('[data-siini-card-size]')) { var card = trigger.closest('[data-siini-product-card]'); if (card) { event.preventDefault(); event.stopImmediatePropagation(); card.querySelectorAll('[data-siini-card-size]').forEach(function (node) { var active = node === trigger; node.classList.toggle('is-selected', active); node.setAttribute('aria-pressed', active ? 'true' : 'false'); }); render(); } return; }
